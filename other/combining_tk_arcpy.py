...
def Model():  # Model
    ...
    IDW = os.path.join(arcpy.env.workspace, r'C:\777p1\ArcGISPro\Geog777_P1\Geog777_P1.gdb\nitrate_IDW')  # Use a fixed name
    nitrate_IDW_result = arcpy.sa.Idw(well_nitrate_OG, "nitr_ran", "0.017616319278512", kValue, "VARIABLE 12", "")
    nitrate_IDW_result.save(IDW)

    # Verify that the dataset was created successfully
    #if arcpy.Exists(IDW):
        # Save IDW result to a layer file
        #arcpy.management.SaveToLayerFile(nitrate_IDW_result, r"C:\777p1\ArcGISPro\Geog777_P1\IDW_SaveToLayerFile.lyrx", "ABSOLUTE")
    #else:
       # print(f"Error: The dataset {IDW} was not created successfully.")
    ...
    Output_Join_Layer = ""
    arcpy.sa.ZonalStatisticsAsTable(cancer_tracts_OG, "GEOID10", nitrate_IDW_result, CancerZonalStats, "DATA", "ALL", "CURRENT_SLICE", [90], "AUTO_DETECT", "ARITHMETIC", 360, Output_Join_Layer)
    ...
    # Save OLS result to a layer file
    arcpy.management.SaveToLayerFile(OLS_fc_shp, r"C:\777p1\ArcGISPro\Geog777_P1\OLS_fc_SaveToLayerFile.lyrx", "ABSOLUTE")

    # Saving PDF of OLS map to the project folder for Layout 1 and Map 1
    ...
    # For Map 1 and Layout 1
    m1 = aprx.listMaps("Map")[0]
    lyrFile1 = arcpy.mp.LayerFile(r"C:\777p1\ArcGISPro\Geog777_P1\OLS_fc_SaveToLayerFile.lyrx")
    m1.addLayer(lyrFile1, "TOP")

    lyt1 = aprx.listLayouts("Layout")[0]
    lyt1.exportToPDF(r"C:\777p1\OLS_Map.pdf")
    print("OLS Map PDF for Layout 1 exported successfully")

    # For Map 2 and Layout 2
    m2 = aprx.listMaps("Map2")[0]
    lyrFile2 = arcpy.mp.LayerFile(r"C:\777p1\ArcGISPro\Geog777_P1\IDW_SaveToLayerFile.lyrx")
    m2.addLayer(lyrFile2, "TOP")

    # Check if "Layout 2" exists
    layouts = aprx.listLayouts("Layout2")
    if layouts:
        lyt2 = layouts[0]
        lyt2.exportToPDF(r"C:\777p1\IDW_Map.pdf")
        print("OLS Map PDF for Layout 2 exported successfully")
    else:
        print("Layout 2 not found in the project")
